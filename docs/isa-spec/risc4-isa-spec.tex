...
% ============================================================
\usepackage{xcolor}
\definecolor{chaptercolor}{RGB}{191,87,0}  % Warm burnt orange
% Alternative warm colors:
% \definecolor{chaptercolor}{RGB}{178,34,34}  % Firebrick red
% \definecolor{chaptercolor}{RGB}{205,92,92}  % Indian red
% \definecolor{chaptercolor}{RGB}{184,134,11} % Dark goldenrod
...
\makechapterstyle{spacer}{%
  \chapterstyle{default}
  \renewcommand*{\printchaptername}{%
    \pagecolor{chaptercolor}%
    \begin{flushright}
    {\Large\bfseries\color{white} CHAPTER}
    \end{flushright}
  }
  \renewcommand*{\chapternamenum}{\par\vspace{0.5cm}}
  \renewcommand*{\printchapternum}{%
    \begin{flushright}
    {\fontsize{60}{72}\selectfont\color{white}\thechapter}
    \end{flushright}
  }
  ...
  \renewcommand*{\afterchapternum}{\par\vspace{1cm}}
  \renewcommand*{\afterchaptertitle}{%
    \par\vfill
    \clearpage
    \pagecolor{white}%  % Reset page color after chapter page
  }
}
\chapterstyle{spacer}

% Section spacing - left justified
\setsecheadstyle{\Large\bfseries\raggedright}
\setbeforesecskip{12pt plus 4pt minus 2pt}
\setaftersecskip{6pt plus 2pt minus 2pt}

\setsubsecheadstyle{\large\bfseries\raggedright}
\setbeforesubsecskip{10pt plus 3pt minus 2pt}
\setaftersubsecskip{4pt plus 2pt minus 2pt}
...
% Force all paragraphs to be left-aligned
...
% ============================================================
% Remove the blank page before TOC title
\renewcommand{\tocheadstart}{}
% Make TOC title smaller and less spaced
\renewcommand{\printtoctitle}[1]{\LARGE\bfseries #1}
\renewcommand{\aftertoctitle}{\par\vspace{1cm}}
...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={RISC-4 ISA Specification v0.2.0},
    pdfauthor={Jeremy King}
}

% Code listings - after hyperref
\usepackage{listings}
...
\lstdefinelanguage{RISC4}{
    morekeywords={ADD, SUB, AND, OR, XOR, SLT, SHF, ADC, SBB, NEG,
                  ADDI, ANDI, ORI, SLTI,
                  LW, SW,
                  BEQ, BNE, BCS, BCC,
                  J, JAL,
...
{\Huge\bfseries RISC-4}\\[0.5cm]
{\LARGE Instruction Set Architecture}\\[0.5cm]
{\large Specification Document}\\[2cm]

{\Large Version \specversion}\\[0.5cm]
{\large \specstatus}\\[3cm]

{\large Jeremy King}\\[0.3cm]
{\large \specdate}\\[2cm]

\vfill

{\large A 4-bit RISC Processor\\
for Educational Implementation in SKY130}
...
Memory Architecture & Von Neumann(unified instruction/data) \\
...
\reg{1}  & ra_h   & Return address [11:8] \\
\reg{2}  & ra_m   & Return Address [7:4] \\
\reg{3}  & ra_l   & Return Address [3:0] \\
\reg{4}  & a0   & Argument 0 / pointer high \\
\reg{5}  & a1   & Argument 1 / pointer mid \\
\reg{6}  & a2   & Argument 2 / pointer low \\
\reg{7}  & t0   & Temp / pointer high \\
\reg{8}  & t1   & Temp / pointer mid \\
\reg{9}  & t2   & Temp / pointer low \\
\reg{10} & fp_h   & Frame pointer [11:8] \\
\reg{11} & fp_m   & Frame pointer [7:4] \\
\reg{12} & fp_l   & Frame pointer [3:0] \\
\reg{13} & sp_h   & Stack pointer [11:8] \\
\reg{14} & sp_m   & Stack pointer [7:4] \\
\reg{15} & sp_l   & Stack pointer [3:0] \\
\bottomrule
\end{tabular}
...
\begin{tabular}{@{}ccccc@{}}
\toprule
\textbf{Pointer} & \textbf{High} & \textbf{Mid} & \textbf{Low} & \textbf{Usage} \\
\textbf{Name} & \textbf{Nibble} & \textbf{Nibble} & \textbf{Nibble} & \\
\midrule
Triple 1  & r1  & r2  & r3  & Return Address \\
Triple 4  & r4  & r5  & r6  & General Purpose / Args \\
Triple 7  & r7  & r8  & r9  & General Purpose / Temps \\
Triple 10 & r10 & r11 & r12 & Frame Pointer \\
Triple 13 & r13 & r14 & r15 & Stack Pointer \\
\bottomrule
\end{tabular}
\caption{Register Triple Organization}
\label{tab:pairs}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Formation:} \texttt{Address = (r[base] << 8) | r[base+1] << 4 | r[base+2]}.
...
\begin{bytefield}[bitwidth=1.5em, endianness=big]{16}
    ...
    \bitbox{4}{rt} & \bitbox{4}{rs} & \bitbox{4}{rd} & \bitbox{4}{\textbf{Opcode}}
\end{bytefield}

\vspace{0.7cm}

% I-Type (Arithmetic)
\textbf{I-Type (Arith)}\\[0.3cm]
\begin{bytefield}[bitwidth=1.5em, endianness=big]{16}
    \bitheader{0,3,4,7,8,11,12,15} \\
    \bitbox{4}{imm4} & \bitbox{4}{rs} & \bitbox{4}{rd} & \bitbox{4}{\textbf{Opcode}}
\end{bytefield}

\vspace{0.7cm}

% I-Type (Branch)
\textbf{I-Type (Branch)}\\[0.3cm]
\begin{bytefield}[bitwidth=1.5em, endianness=big]{16}
    \bitheader{0,7,8,11,12,15} \\
    \bitbox{8}{offset8} & \bitbox{4}{cond} & \bitbox{4}{\textbf{Opcode}}
\end{bytefield}
...
\begin{bytefield}[bitwidth=1.5em, endianness=big]{16}
    \bitheader{0,3,4,7,8,11,12,15} \\
    \bitbox{4}{offset4} & \bitbox{4}{base} & \bitbox{4}{rd / rs} & \bitbox{4}{\textbf{Opcode}}
\end{bytefield}
...
\begin{bytefield}[bitwidth=1.5em, endianness=big]{16}
    ...
    \bitbox{12}{target12} & \bitbox{4}{\textbf{Opcode}}
\end{bytefield}
...
ADC rd, rs
\end{lstlisting}
\instructionDef{7}{R-type}{0111 dddd ssss 0000}{rd \leftarrow rd + rs + Carry}{C, Z}{4}
...
SBB rd, rs
\end{lstlisting}
\instructionDef{7}{R-type}{0111 dddd ssss 0001}{rd \leftarrow rd - rs - Carry}{C, Z}{4}
...
\instructionDef{F}{J-type}{1111 0ttt tttt tttt}{PC \leftarrow \text{target12}}{--}{1}

\noindent \textbf{Description:} Unconditional jump to the 12-bit absolute address specified by \texttt{target12}. The full instruction memory address space (0x000-0xFFF) is reachable.

\textbf{Flags:} No flags are modified.

\textbf{Examples:}
\begin{lstlisting}
J 0x234    # Jump to address 0x234
J 0xFFE    # Jump to address 0xFFE (near end of memory)
...
\begin{itemize}
    \item Infinite loops
    \item Far jumps (across pages)
    \item End of non-returning functions
    \item Jump tables (with computed addresses)
\end{itemize}

\subsection{JAL -- Jump And Link}
\begin{lstlisting}
JAL target11
\end{lstlisting}

\instructionDef{F}{J-type (variant)}{1111 1ttt tttt tttt}{r1 \leftarrow PC[11:8], r2 \leftarrow PC[7:4], r3 \leftarrow PC[3:0], PC \leftarrow \{1'b0, \text{target11}\}}{--}{1}

\noindent \textbf{Description:} Jump and Link performs an unconditional jump to the 11-bit target address while saving the return address across three registers (r1:r2:r3). This instruction is used for function calls.

\textbf{Register Convention:} The return address is split across three 4-bit registers:
\begin{itemize}
    \item r1 receives PC[11:8] (high nibble)
    \item r2 receives PC[7:4] (middle nibble)
    \item r3 receives PC[3:0] (low nibble)
\end{itemize}

\textbf{Address Range:} JAL can reach addresses 0x000-0x7FF (2048 nibbles, 1024 bytes). For functions beyond this range, use J (unconditional jump) or reorganize code placement.
...
main:
    ORI  r4, r0, 5      # Argument in r4
    JAL  add_one        # Call function, r1:r2:r3 <- return address
    # Execution continues here after return

add_one:
    ADDI r4, r4, 1      # r4 = r4 + 1
    JR   r1             # Return (see JR instruction)
\end{lstlisting}

\textbf{Nested Function Call Example:}
\begin{lstlisting}
caller:
    JAL  func_a         # r1:r2:r3 <- return address (e.g., 0x010)

func_a:
    # Save return address to stack before calling another function
    ADDI r15, r15, -3   # Allocate stack space
    BCC  no_borrow
    ADDI r14, r14, -1
no_borrow:
    SW   r1, 0(r14)     # Save r1 (high nibble)
    SW   r2, 1(r14)     # Save r2 (mid nibble)
    SW   r3, 2(r14)     # Save r3 (low nibble)

    JAL  func_b         # Call another function (overwrites r1:r2:r3)
...
no_carry:
    ...
    JR   r1             # Return to original caller
\end{lstlisting}

\textbf{Pipeline Behavior:}

JAL executes in the ID stage:
\begin{itemize}
    \item \textbf{Cycle 1 (IF):} Fetch JAL instruction
    \item \textbf{Cycle 2 (ID):}
    \begin{itemize}
        \item Decode JAL
        \item Write PC+1 to r1:r2:r3 (three register writes)
        \item Update PC to target
        \item Flush IF stage (wrong instruction was speculatively fetched)
    \end{itemize}
    \item \textbf{Cycle 3 (IF):} Fetch instruction at target address
\end{itemize}

Total latency: 2 cycles (1 for JAL + 1 for branch penalty)

\textbf{Design Notes:}
\begin{itemize}
    \item JAL performs three simultaneous register writes. Implementation must support writing r1, r2, and r3 in the same cycle, or execute JAL over multiple cycles.
    \item The target address has bit [11] forced to 0, limiting range to the lower half of instruction memory.
    \item For calls to addresses beyond 0x7FF, use a two-instruction sequence: J to the target, then save return address manually.
\end{itemize}
...
\instructionDef{7}{R-type (Extended)}{0111 xxxx xxxx 0011}{PC \leftarrow \{r1, r2, r3\}}{--}{1}

\noindent \textbf{Description:} Jump Register performs an indirect jump to the 12-bit address formed by concatenating registers r1, r2, and r3. This instruction is used to return from function calls.

\textbf{Register Convention:} JR always reads from r1:r2:r3 (the return address registers). The encoding fields for rd and rs are unused (set to 0 by assembler).

\textbf{Address Formation:}
\begin{lstlisting}[language={}]
PC <- {r1[3:0], r2[3:0], r3[3:0]}
       |         |         |
       High      Mid       Low
\end{lstlisting}
...
\textbf{Examples:}
\begin{lstlisting}
# Simple return from function:
add_one:
    ADDI r4, r4, 1      # Do work
    JR   r1             # Return to caller

# Return after nested call (r1:r2:r3 restored from stack):
func_a:
    # ... work ...
    LW   r1, 0(r14)     # Restore return address from stack
    LW   r2, 1(r14)
    LW   r3, 2(r14)
    JR   r1             # Return
\end{lstlisting}
...
JR executes in the ID stage with dedicated return address path:
\begin{itemize}
    \item \textbf{Cycle 1 (IF):} Fetch JR instruction
    \item \textbf{Cycle 2 (ID):}
    \begin{itemize}
        \item Decode JR
        \item Read r1, r2, r3 via dedicated 12-bit concatenation path
        \item Update PC to \{r1, r2, r3\}
        \item Flush IF stage
    \end{itemize}
    \item \textbf{Cycle 3 (IF):} Fetch instruction at return address
\end{itemize}

Total latency: 2 cycles (1 for JR + 1 for branch penalty)

\textbf{Hazard Handling:}

If a previous instruction modifies r1, r2, or r3, forwarding logic provides the updated values:
\begin{lstlisting}
# Hazard example:
ADD  r1, r4, r5     # r1 written in WB stage
JR   r1             # r1 needed in ID stage
# Forwarding resolves this with 0 stalls
\end{lstlisting}

The dedicated return address path includes forwarding muxes for r1, r2, and r3 from the EX/MEM and MEM/WB pipeline registers.

\textbf{Design Notes:}
\begin{itemize}
    \item JR uses a dedicated 12-bit read path that always accesses r1:r2:r3, separate from the normal register file read ports.
    \item This dedicated path allows JR to execute in a single cycle without requiring a 3-port register file.
    \item Hardware cost: ~50 gates (ASIC) or 10 LUTs (FPGA) for the concatenation and forwarding logic.
\end{itemize}
% ============================================================
...
\textbf{Critical Path:}
\begin{lstlisting}[language={}]
PC \rightarrow Instruction Memory \rightarrow Instruction Register \rightarrow IF/ID Register
\end{lstlisting}
...
\begin{lstlisting}[language={}]
1. EX/MEM.ALUResult $\rightarrow$ ID/EX (ALU operand)
2. MEM/WB.Result $\rightarrow$ ID/EX (ALU operand)
\end{lstlisting}
...
Step 1: r1 = r3 + r5 = 0xF + 0x3 = 0x12 $\rightarrow$ r1 = 0x2, C = 1
Step 2: r0 = r2 + r4 + C = 0x9 + 0x2 + 1 = 0xC
Result: r0:r1 = 0xC2 (194 decimal) $\checkmark$
\end{lstlisting}
...
Step 1: r1 = r3 - r5 = 0x4 - 0x7 = -3 $\rightarrow$ r1 = 0xD (13), C = 1 (borrow)
Step 2: r0 = r2 - r4 - C = 0x8 - 0x3 - 1 = 0x4
Result: r0:r1 = 0x4D (77 decimal) $\checkmark$
\end{lstlisting}
...
\begin{lstlisting}[language={}]
High addresses
    [local variable n]  $\leftarrow$ FP + n
    [local variable 2]  $\leftarrow$ FP + 2
    [local variable 1]  $\leftarrow$ FP + 1
    [saved r12]         $\leftarrow$ FP + 0  (old frame pointer)
    [return address]    $\leftarrow$ FP - 1
    [saved registers]   $\leftarrow$ FP - 2, FP - 3, ...
    [free space]        $\leftarrow$ SP (current stack pointer)
Low addresses
\end{lstlisting}
...
r2 & ra\_hi & Return address HIGH & Caller Saves (if nested call) \\
r3 & ra\_mid & Return address MID & Caller Saves (if nested call) \\
r4 & ra\_low & Return address LOW 2 & Caller Saves (if nested call) \\
r5 & a3 & Argument 3 & No \\
...
\subsection{Return Address Handling}
\textbf(For leaf functions)(don't call other functions):
The return address in r1:r2:r3 is automatically preserved. Simply end with JR r1
\textbf(For non-leaf functions)(call other functions):
Save r1:r2:r3 to the stack before making calls, restore before returning.
...
\midrule
r4 (a0) & First argument (or bits [3:0] of multi-nibble value) \\
r5 (a1) & Second argument (or bits [7:4] of multi-nibble value) \\
r6 (a2) & Third argument \\
r7 (a3) & Fourth argument \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}
# Call: result = add_three(5, 3, 2)

ORI  r4, r0, 0x5    # Set argument: a0 = 5
ORI  r5, r0, <ret_addr_high>  # Load return address
ORI  r6, r0, <ret_addr_low>   # (simplified; real implementation uses computed address)
JAL    add_three        # Call function

add_three:
    ADD r4, r4, r5 # a0 = a0 + a1
    ADD r4, r4, r6 # a0 = a0+a2
    JR r1          # Return
# Return here
\end{lstlisting}
...
High addresses
    [Caller's frame]
    [................]
    [Argument n]        $\leftarrow$ FP + n (arguments beyond 4)
    [Argument 5]        $\leftarrow$ FP + 5
    [Return address]    $\leftarrow$ FP + 4
    [Saved FP high]     $\leftarrow$ FP + 3
    [Saved FP low]      $\leftarrow$ FP + 2
    [Saved r10]         $\leftarrow$ FP + 1
    [Saved r11]         $\leftarrow$ FP + 0 (FP points here)
    [Local var 0]       $\leftarrow$ FP - 1
    [Local var 1]       $\leftarrow$ FP - 2
    [Temp storage]      $\leftarrow$ SP (current stack pointer)
Low addresses
\end{lstlisting}
...
add_three:
    ...
    # Function body
    ADD r4, r5, r6      # r4 = a + b
    ADD r4, r5, r6      # r4 = a + b + c

    # No epilogue needed
    JR r1               # return to r1 address
\end{lstlisting}
...
\begin{lstlisting}
# Function: add_and_double(x, y) -> (x + y) * 2
# Calls another function: double(n)
...
add_and_double:
  ...
  ADDI r14, r14, -1
no_borrow_p:
  SW   r1, 0(r14)     # Save ra_hi
  SW   r2, 1(r14)     # Save ra_mid
  SW   r3, 2(r14)     # Save ra_low

  # Function body
  ADD  r4, r4, r5     # x = x + y
  JAL  double         # Call double(x+y), overwrites r1:r2:r3
  # Result in r4

  # Epilogue: Restore return address
  LW   r1, 0(r14)     # Restore ra_hi
  LW   r2, 1(r14)     # Restore ra_mid
  LW   r3, 2(r14)     # Restore ra_low
  ADDI r15, r15, 3    # Deallocate stack
  BCC  no_carry_e
  ADDI r14, r14, 1
no_carry_e:
...
r1 & ra\_hi/ra(or full return address alias) \\
r2 & ra\_mid & Return address MID \\
r3 & ra\_low & Return address LOW \\
r4 & a2 & Argument 2 \\
r5 & a3 & Argument 3 \\
...
\midrule
0.1.0 & 2026-01-21 & Major documentation expansion \\
      &            & - Added Chapter 4: Status Flags (comprehensive) \\
      &            & - Added Chapter 6: Pipeline Architecture (5-stage pipeline) \\
      &            & - Expanded Chapter 7: Addressing (register pairs, multi-precision) \\
      &            & - Expanded Chapter 8: Calling Convention (complete prologue/epilogue) \\
...
\midrule
0.2.0 & 2026-01-22 & Function call mechanism and ISA refinement \\
      &            & - Added JAL (Jump And Link) instruction \\
      &            & - Added JR (Jump Register) instruction \\
      &            & - Implemented 12-bit return address using r1:r2:r3 register triple \\
      &            & - Updated J-type format to support JAL variant (bit [11] as link flag) \\
      &            & - Extended EXT opcode with JR function (code 0x3) \\
      &            & - Revised calling convention (r4-r7 now argument registers) \\
      &            & - Updated register file documentation (r1:r2:r3 reserved for returns) \\
      &            & - Added dedicated return address path in pipeline \\
      &            & - Documented leaf vs non-leaf function prologue requirements \\
      &            & - Added compiler guidance for JAL range limitation (11-bit) \\
      &            & - Added trampoline strategy for far function calls \\
\bottomrule
\end{tabular}
\caption{Document Revision History}
\end{table}

\section{Version 0.0.1 (Initial Draft)}
...
\section{Version 0.1.0 (Documentation Expansion)}
...
\subsection{Chapter 6: Pipeline Architecture}
\begin{itemize}
...
\subsection{Chapter 7: Addressing (Expanded)}
\begin{itemize}
...
\subsection{Chapter 8: Calling Convention (Expanded)}
\begin{itemize}
...
\section{Version 0.2.0 (Function Call Mechanism)}

\textbf{Date:} January 22, 2026
...
\begin{itemize}
    \item \textbf{JAL (Jump And Link):} Unconditional jump with return address save
    \begin{itemize}
        \item Encoding: \texttt{1111 1ttt tttt tttt} (J-type variant, bit [11] = 1)
        \item Operation: \texttt{r1 ← PC[11:8], r2 ← PC[7:4], r3 ← PC[3:0], PC ← target11}
        \item Range: 0x000--0x7FF (2048 nibbles, 11-bit target)
        \item Uses register triple (r1:r2:r3) for 12-bit return address storage
    \end{itemize}
    \item \textbf{JR (Jump Register):} Indirect jump to address in r1:r2:r3
    \begin{itemize}
        \item Encoding: \texttt{0111 xxxx xxxx 0011} (EXT function code 0x3)
        \item Operation: \texttt{PC ← \{r1, r2, r3\}} (concatenate to form 12-bit address)
        \item Dedicated hardware path for return address read
        \item Single-cycle execution with forwarding support
    \end{itemize}
\end{itemize}

\subsection{Calling Convention Overhaul}
\begin{itemize}
    \item \textbf{Return address mechanism:} r1:r2:r3 form 12-bit return address register
    \begin{itemize}
        \item r1 (ra\_hi): High nibble [11:8]
        \item r2 (ra\_mid): Middle nibble [7:4]
        \item r3 (ra\_low): Low nibble [3:0]
    \end{itemize}
    \item \textbf{Argument registers shifted:} r4--r7 now used for parameters (was r2--r5)
    \item \textbf{Return value registers:} r4, r6, r7 (was r2, r6)
    \item \textbf{Leaf function optimization:} No prologue/epilogue required for functions that don't call others
    \item \textbf{Non-leaf function convention:} Must save r1:r2:r3 to stack before nested calls
\end{itemize}

\subsection{Pipeline Enhancements}
\begin{itemize}
    \item Dedicated 12-bit return address read path in ID stage
    \item Forwarding logic for r1, r2, r3 to handle hazards on JR
    \item JAL executes in ID stage with three simultaneous register writes
    \item JR hazard handling documented (forwarding from EX/MEM/WB stages)
\end{itemize}

\subsection{Instruction Format Updates}
\begin{itemize}
    \item J-type format split into two variants based on bit [11]:
    \begin{itemize}
        \item Bit [11] = 0: J (unconditional jump, 12-bit range)
        \item Bit [11] = 1: JAL (jump and link, 11-bit range)
    \end{itemize}
    \item EXT opcode function code allocation updated:
    \begin{itemize}
        \item 0x0: ADC (existing)
        \item 0x1: SBB (existing)
        \item 0x2: NEG (existing)
        \item 0x3: JR (new)
        \item 0x4--0xF: Reserved for future use
    \end{itemize}
\end{itemize}

\subsection{Documentation Additions}
\begin{itemize}
    \item Complete JAL/JR instruction specifications with examples
    \item Leaf vs non-leaf function prologue/epilogue decision tree
    \item Nested function call examples with stack management
    \item Tail call optimization patterns
    \item Register usage convention table updated
    \item Pseudo-instruction RET added (expands to JR r1)
\end{itemize}

\subsection{Compiler Guidance}
\begin{itemize}
    \item JAL range limitation documented (11-bit = 2048 nibble max)
    \item Trampoline strategy for calling functions beyond 0x7FF
    \item Code layout optimization recommendations
    \item Hot function placement in low memory (JAL-reachable range)
    \item Linker responsibilities for trampoline generation
\end{itemize}

\textbf{Breaking Changes from v0.1.0:}
\begin{itemize}
    \item Calling convention incompatible with v0.1.0 (argument registers changed)
    \item r1, r2, r3 now reserved for return addresses (not general-purpose)
    \item Function prologue requirements changed (leaf functions need no prologue)
\end{itemize}

\textbf{Known Limitations:}
\begin{itemize}
    \item JAL limited to 11-bit range (first 2048 nibbles of instruction memory)
    \item Functions beyond 0x7FF require trampoline mechanism
    \item Return address uses 3 registers (reduces general-purpose register count)
    \item No PC-relative call instruction (all calls are absolute addressing)
\end{itemize}

\textbf{Remaining for v1.0:}
\begin{itemize}
    \item Consider JALR with PC-relative addressing (±128 nibble range)
    \item Investigate full 12-bit JAL encoding options
    \item Additional example programs demonstrating call/return patterns
    \item Hardware verification of dedicated return address path
    \item Compiler implementation with trampoline generation
    \item Linker specification for code layout optimization
\end{itemize}
...
\begin{itemize}
    \item JALR instruction with PC-relative addressing (under consideration)
    \item Full 12-bit JAL range if encoding space found
    \item Hardware verification complete
    \item Tapeout-ready RTL
    \item Complete test suite
    \item Assembler and toolchain with trampoline support
